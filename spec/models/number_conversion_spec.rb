...
RSpec.describe NumberConversion, type: :model do
  ...
  it "should convert numbers to hex" do
  ...
  it "should disallow invalid numbers" do
    expect(NumberConversion.new(number:-1, base:2)).to be_invalid
    expect(NumberConversion.new(number:-40, base:2)).to be_invalid
    expect(NumberConversion.new(number:'abc', base:2)).to be_invalid
  end

  it "should disallow invalid bases" do
    expect(NumberConversion.new(number:9, base:0)).to be_invalid
    expect(NumberConversion.new(number:-1, base:1)).to be_invalid
    expect(NumberConversion.new(number:3, base:'abc')).to be_invalid
  end
